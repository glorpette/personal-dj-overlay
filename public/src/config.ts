...
export const CONFIG_PATH = resolve(process.cwd(), process.env.VDJ_OVERLAY_CONFIG || "config.json");
...
export function loadConfig(): AppConfig {
  if (!existsSync(CONFIG_PATH)) {
    log.warn(`No config.json at ${CONFIG_PATH} — writing defaults from config.example.json`);
    saveConfig(DEFAULTS);
